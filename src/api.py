...
try:
    ...
    pip.main(["install", "python-dotenv", "-q"])
    reload(site)
    from fastapi import FastAPI, Request
    from pymongo import MongoClient
    import uvicorn
...
def process (input, frame_cap, quality, color_weight, focus_weight, farm, threads, db_address):
    ...
    if farm is False:
        ...
        package_path = os.environ.get("RUN_FRAME_ANALYSIS", os.path.abspath(inspect.getmodule(main).__file__))
        ...
        layers ={
                ...
                "kwargs": {
                    ...
                    "command": f"rez-env ffmpeg -- {os.environ["RUN_FRAME_ANALYSIS"]} '{json.dumps(args)}'"
                },
                "settings": {
                    "start": 1001,
                    "end": 1001,
...
def parallel (input, frame_cap = 100, quality = 0.5, color_weight = -1, focus_weight = -1, farm = True, priority = 100, farm_threads = 4, 
             db_address = [os.environ["MONGO_URL"],"frame_analysis","thumbnail_gen"], cue_address = [os.environ["CUE_API_URL"], os.environ["CUE_version"],], submission_threads = None):
    ...
    if type(input) is str:
        results_tuple = [process(input, frame_cap, quality, color_weight, focus_weight, farm, farm_threads, db_address)]
        results = [results_tuple[0][0]]
        single = True
    else:
        with ProcessPoolExecutor(submission_threads) as exe:
            futures = [exe.submit(process, i, frame_cap, quality, color_weight, focus_weight, farm, farm_threads, db_address) for i in input]
