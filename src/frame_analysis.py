...
def get_paths(input, store_quality):
    ...
    if len(path.suffixes) == 1:
        ...
        if num_frames is None:
            ...
            a_frames, store_quality, num_frames = get_quality(
                a_frames, store_quality, num_frames
            )
        fps = round(vid.fps)
    ...
    for f, frame in enumerate(frame_list):
        ...
        if len(a_frames) == 0:
            ...
            # File paths from sequence
                ...
                check = (
                    assembly.head + str(frame).zfill(assembly.padding) + assembly.tail
                )
            a_frames.pop(0)
...
def aces_srgb(read):
    ...
    if not aces or not srgb:
        raise ValueError(
            "Could not find the appropriate color spaces in the OCIO configuration"
        )
    ...
    converted_rgb_8bit = cv2.cvtColor(
        (converted_rgb * 255).astype(np.uint8), cv2.COLOR_RGB2BGR
    )
...
def main(input, store_quality, color_weight, focus_weight, db_address, id):
    ...
    collection = (
        MongoClient(db_address[0])
        .get_database(db_address[1])
        .get_collection(db_address[2])
    )
